package com.scheffer.erik.popularmovies.moviedatabaseapi;

import com.scheffer.erik.popularmovies.BuildConfig;

public class ApiConstants {
    static final String MOVIES_DATABASE_API_KEY = BuildConfig.MOVIE_API_KEY;
    static final String MOVIES_DATABASE_BASE_URL = "https://api.themoviedb.org/3/movie/";
    public static final String MOVIES_DATABASE_BASE_POSTER_URL = "http://image.tmdb.org/t/p/";
}
